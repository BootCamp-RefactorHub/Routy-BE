...
| 곽동근 |  |
| 김승민 |  |
| 변민철 | 이번 프로젝트는 DB 모델링부터 CI/CD 구축까지 전체 개발 프로세스를 직접 경험한 프로젝트로, 개발자로서 기술적 역량을 체계적으로 확장할 수 있었던 의미 있는 기간이었다. 이전 프로젝트에서는 규모 대비 역량 부족으로 인해 여러 제약을 겪었지만, 이번에는 프로젝트 범위가 적정하여 부족한 기술을 학습하면서 구현까지 이어가는 유의미한 흐름을 만들 수 있었다. <br><br> 특히 기술적으로는 Spring Security와 JWT 기반 인증·인가, Docker–Kubernetes–Argo CD를 활용한 CI/CD 자동화 파이프라인 구축을 통해 백엔드와 인프라 전반에 대한 이해도를 크게 높였다. 이를 통해 서비스 배포 프로세스의 자동화, 운영 환경 구조, 컨테이너 오케스트레이션 등 인프라 영역에 흥미와 도전 의식을 갖게 되었다. <br><br> 프로젝트는 짧은 기간의 해커톤 방식으로 진행되어 규모가 다소 작았고, 새로운 기술을 과감히 도입할 기회가 제한적이었다는 아쉬움도 있었다. 또한 아이디어 제안 시 근거나 대안 제시가 부족해 팀을 충분히 설득하지 못한 경험을 통해, 기술적 판단의 논리성 및 커뮤니케이션 역량의 중요성을 다시 한 번 느꼈다. <br><br>그럼에도 불구하고, 이번 프로젝트는 이전 프로젝트에서 발생했던 문제를 개선하기 위해 기획 단계부터 팀 규칙과 협업 방식을 정립하고, 이를 지속적으로 유지하며 운영했다는 점에서 큰 의미가 있다. 이 과정에서 협업 시 필요한 소프트스킬의 중요성을 체감했으며, 기술적 역량과 함께 균형 있게 성장할 수 있는 기반이 마련되었다.<br><br>마지막으로, 이번 경험을 통해 동시성 제어, 실시간 처리, 트랜잭션 처리, 서비스 통신 구조 등 백엔드 개발자로서 더 깊게 다뤄야 할 영역이 분명해졌고, 이를 파이널 프로젝트에서 보완하고 확장하고자 한다. 앞으로도 기술적 하드 스킬과 협업 역량을 함께 강화하는 방향으로 성장해 나갈 예정이다. |
| 이승건 |  |
