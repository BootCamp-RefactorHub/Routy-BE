...
| 변민철 |  |
| 이승건 |  |
| 이지윤 | 이번 프로젝트에서는 마이페이지 기능을 중심으로 개발을 진행했다. 백엔드와 프론트엔드 간의 데이터 흐름을 MyBatis 기반으로 구성했고, 작성된 일정이 마이페이지뿐만 아니라 브라우저 페이지에서도 정확하게 표시되도록 구현했다. 일정 목록만 보여주는 것이 아닌, 선택한 일정이 달력에 반영되도록 연결하여 사용자 입장에서 일정 관리가 더욱 직관적으로 이루어질 수 있도록 설계하였다.
다른 사용자들이 공개한 일정도 적극적으로 활용할 수 있도록 ‘북마크’ 기능과 ‘내 일정으로 불러오기’ 기능을 추가했다. 이를 통해 공유된 일정이 단순한 정보 제공에서 끝나는 것이 아니라, 실제 사용자의 계획에 자연스럽게 녹아들 수 있도록 기능성을 확보했다. MyBatis를 사용할 때는 서비스 계층과 XML 매퍼를 인터페이스로 연결해 결합도를 낮추고, 데이터베이스 스키마가 변경되더라도 서비스 로직에 영향이 최소화되도록 구조를 설계했다. 이러한 구조적 설계 덕분에 중간에 컬럼 변경이 있었도 큰 문제 없이 대응할 수 있었다. 다만 이번 프로젝트에서는 MyBatis 중심의 개발을 진행하면서 JPA 기반의 업데이트나 복잡한 수정 기능을 직접 다뤄보지 못한 점이 아쉬움으로 남았다. 추후 기회가 된다면 JPA 기반으로 수정 페이지까지 완성해보고 싶다는 목표도 생겼다.
프론트엔드에서는 API 경로 충돌이 발생하지 않도록 요청 주소를 세심하게 설계했고, 기존 팀원들이 만들어 둔 디자인 흐름과 컴포넌트 스타일을 유지하기 위해 UI 일관성에도 신경 쓰며 작업했다. 저번 프로젝트에서는 의견 조율 과정에서 충분히 소통하지 못했던 경험이 있어, 이번에는 팀원들의 의견을 먼저 듣고 조율하는 방식으로 협업 태도를 개선했다. 디자인 통일성, 컬럼명 조정, 데이터 흐름 정리 등 다양한 부분에서 서로 의견을 교환하며 프로젝트를 안정적으로 맞춰 나갈 수 있었다.
코드 리뷰 과정에서도 이전보다 훨씬 깊이 있게 참여했다. 변경된 코드의 목적과 의도를 먼저 이해한 뒤 피드백을 남기고, 잘 구현된 부분은 칭찬하며 유지보수성과 읽기 쉬운 코드 기반을 함께 만들어 갔다. 단순히 PR을 보내는 것이 아니라, 충돌을 예방하고 서로 배움을 나누는 환경을 만들었다는 점에서 개인적으로도 큰 성장이라고 느꼈다.
이번 프로젝트는 단순 기능 구현을 넘어, 협업·커뮤니케이션·구조 설계·UI 일관성·데이터 흐름 관리 등 다양한 영역에서 실제 개발 과정이 어떻게 이어져야 하는지 몸으로 느낄 수 있었던 경험이었다. 남은 아쉬움도 있지만, 그만큼 다음 프로젝트에서 개선할 방향도 분명해졌고 전체적으로 성장의 폭이 컸던 의미 있는 작업이었다.
  |
| 최혜원 |  |
